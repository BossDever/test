...
try {
    ...
    $sql = "SELECT q.id as qid, q.item_code, q.section_id, a.value_likert, r.id as response_id, s.code as section_code, s.title as section_title
                FROM answers a
    ...
    while ($row = $stmt->fetch()) {
        ...
        if (isset($byQuestion[$qid]['dist'][$v])) $byQuestion[$qid]['dist'][$v]++;
        $all[] = $v;
    ...
    $dims[$sec]['label'] = ($row['section_code'] ? ($row['section_code'].' ') : '') . ($row['section_title'] ?? ('Section '.$sec));
    }
    ...
    $items=[]; foreach($byQuestion as $qid=>$info){ $items[] = [ 'qid'=>$qid, 'code'=>$info['code'], 'avg'=>$avg($info['vals']), 'n'=>count($info['vals']), 'dist'=>$info['dist'] ]; }
    usort($items, fn($a,$b)=>strcmp($a['code'],$b['code']));

    $dimensions = [];
    if (!empty($dims)) {
        foreach ($dims as $secId=>$d) {
            $dimensions[$d['label'] ?? ('Section '.$secId)] = $d['n']? $d['sum']/$d['n'] : 0;
        }
    }
    ...
    // Respondent breakdown (unique response count per type)
    $respBreak = [];
    $sqlResp = 'SELECT respondent_type, COUNT(DISTINCT id) as cnt FROM responses WHERE ' . $where . ' GROUP BY respondent_type';
    $stmt2 = $pdo->prepare($sqlResp);
    $stmt2->execute($args);
    while ($r = $stmt2->fetch()) { $respBreak[$r['respondent_type'] ?: 'unknown'] = (int)$r['cnt']; }

    echo json_encode(['ok'=>true, 'summary'=>$summary, 'items'=>$items, 'dimensions'=>$dimensions, 'respondents'=>$respBreak]);
} catch (Throwable $e) {
